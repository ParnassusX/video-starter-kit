...
import { Hero, Header, Features, Community, Footer } from './mocks/landing-components';
...
describe('End-to-End Workflow', () => {
  ...
  test('Replicate API integration for image generation', async () => {
    ...
    const mockMutateAsync = jest.fn().mockResolvedValue({ id: 'mock-prediction-id' });
  ...
  test('Successful storage of generated content', async () => {
    ...
    render(<App projectId="test-project-id" queryClient={mockQueryClient} />, { wrapper: createWrapper() });
    ...
    await waitFor(() => {
      ...
      expect(mockMediaCreate).toHaveBeenCalledWith(expect.objectContaining({
        projectId: 'test-project-id',
        kind: 'generated',
      }));
    });
    ...
    await waitFor(() => {
      expect(mockInvalidateQueries).toHaveBeenCalledWith(['projectMediaItems', 'test-project-id']);
    });
  ...
  test('Image/video upload functionality', async () => {
    ...
    const mockCreateObjectURL = jest.fn().mockReturnValue('blob:http://localhost:3000/mock-blob-url');
